...
if (isset($_POST['regisIndustri'])) {
    ...
    if ($cek > 0) {
        echo '<script>alert("Gagal Daftar, Username sudah ada");window.location.href = "../../../views/user/formRegis/industri.php";</script>';
        exit;
    } else {
        $crud = new respondenCRUD();
        $user_id = $crud->createAkun($username, $name, $password, $role);
        $crud->createIndustri($user_id, $name, $jabatan, $perusahaan, $email, $nohp, $kota);
